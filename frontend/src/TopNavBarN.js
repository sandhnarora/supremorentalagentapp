...
const TopNavBarN = ({ onLogout, userJsonVal, bookingCount, clearFilters }) => {
  ...
  useEffect(() => {
    if (userJsonVal && userJsonVal.userid) {
      const userId = userJsonVal.userid;
      setUserID(userId);
      // console.log('Fetching data for user ID: > ', userId);   order-service/orders

      fetch(`${process.env.REACT_APP_BACKEND_SERVICE_IP}/order-service/orders?userid=${userId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  if (!getUserId) {
    ...
    return (
      <Navbar  variant="dark" expand="lg" className=' mx-md-4'>
        <div className="container-fluid bg-secondary px-md-5 navBarPos">
        {/* <Container> */}
          <h2 className="ftco-heading-2">
